...
public class AppointmentPage extends JFrame {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
						AppointmentPage frame = new AppointmentPage(alphan, umut);
	 ...
	 * Create the frame.
	 */
	public AppointmentPage(Patient umut, Doctor alphan) {
		...
		for ( int i = 0 ; i < buttons.length ; i++){
			...
			buttons[i].addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					umut.getAppointment(cc, alphan);
					buttons[cc].setBackground(new Color(242, 186, 201));
					buttons[cc].setText ( alphan.getDate().getDay(cc).toString() + " not available");
				}
			}
			 ...
			 if(umut.getAvailabe(i)  && alphan.getAvailability(i)){
				buttons[i].setBackground(new Color(186, 242, 187));
				buttons[i].setOpaque(true);
				buttons[i].setBorderPainted(true);
				buttons[i].setText ( alphan.getDate().getDay(i).toString() + " available");
				...
				buttons[i].setOpaque(true);
				buttons[i].setBorderPainted(true);
				buttons[i].setText ( alphan.getDate().getDay(i).toString() + " not available");
			 }
